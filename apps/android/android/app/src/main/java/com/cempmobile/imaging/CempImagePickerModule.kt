...
class CempImagePickerModule(reactContext: ReactApplicationContext) :
  ...
  fun pick(promise: Promise) {
    ...
    // Install the new promise and reject any prior in-flight pick, atomically.
    val prior = pending.getAndSet(promise)
    prior?.reject("image-pick-cancelled", "superseded by a new pick")
    try {
  ...
  override fun onActivityResult(activity: Activity, requestCode: Int, resultCode: Int, data: Intent?) {
    ...
    Thread {
      try {
        val bytes = readAllBytes(resolver, uri)
